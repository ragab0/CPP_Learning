...
===== CH3: Printing =====
=========================
-The returned value of 0 of main function means that everything is good, there is no error and we're done!;
-Compiler ignores everything after the return statement SUCH AS ignoring commenting;
-Code tracing means going over the code line by line and act like a compiler (analysis for youself) :D;
    -- Read the code;
    -- Descide what the compiler is going to do;
...
-Data type: We vs Programming:
    ●● 52           for us is a Number for C++ is an Integer;
    ●● 12.0         for us is a Number for C++ is a Double (or Float);
    ●● 'Z'          for us is a Letter for C++ is a Character;
    ●● "Computer"   for us is a Word and for C++ is a String (Sequence of characters);
    ●● Male\Female? for us is a Status of 2 things and for C++ is a Boolean (2);
-Computer Memory Like Streets:
    ●● Each home has street address (location);

-Definition: the name of fun or var or the type; 
-Declaration: the details of implementation (stores in memory);
-Initialization a variable (1 +3 Ways):
    -- int age;
    .. "Declaration and definition .. Un-initialized: (Garbage value)";
    -- int age = 50;
    .. "The C-Style The popular";
    -- int age (50);
    .. "Constructor initialization";
    -- int age {50};
    .. "The Modern initialization The preferred";
...
Reading variables
-----------------
-To read we use cin >> A_Defined_VAR;
-Compiler here FREEZES .. waits we;

Constant Variables
------------------
-Define once Can’t change later;
-We use const to:
    -- Disallow a change;
    -- Clear intentions;
...
--------------------------------------
-It is:
    -- A Binary symbol that need two operands [+, -, /, *];
    -- A Unary symbol that need one operand [!, +, -];
    -- A Ternary symbol that need three operands (?:);
-Operand: The Value that operator will apply on [x, y, 1, 2];
-Expression:
    -- The entire statement;
    .. "Which is end with the statement terminator (semicolon)" :D;
...
-Unary Operators: [++, --, !];
-Binary Operators:
    -- Arithmitic Operators (traditional operations): [+, -, /, *, %];
    -- Relational (Comparison) Operators: [<, >, <=, >=, ==, !=];
    -- Logical Operators: [!, ||, &&];
    -- Bitwise Operators: [&, |, <<, >>, ~, ^];
    -- Assignment Operators and (Compound Operators - assignment compound with the binary operators): [=, ];
    -- Comma Operator: (,) :DD;
-Ternary Operator:
...
Prefix(Before) and Postfix(After) Unary Operatorss (++, --)
-----------------------------------------------------------
-Prefix (Before): Inc\Dec Firstly and its priority is heigher;
-BEcarful:
    ## a++ + ++a: This bad behavior Ends with undefined;
    ## a-- + --a: This bad behavior Ends with undefined;
    ## a + ++a: This bad behavior Ends with undefined;
    .. "X + Y: There is no defined order if X evaluated first or Y";
    .. "A side-effect: a change in state from one expression ISN'T ALLOWED";
-Better to use Prefix as default;
-Better to use Postfix whenever you want to copy a variable;
...
-The assignment operator:
    ...
    -- Rhs is an expression ends with the statement terminator (;);
    -- Rhs is evaluated firstly and then assigned to Lhs;
    -- Rhs returns the same type of Lhs otherwise (A Compiler Error);
-The combound assignment operators:
    -- Assignment combound with a binary operator (specific with the arithmitic operators);
-The comma operator:
    -- cout << (10, x=1, y=2, 40) => 40;
    .. "which is evaluate from left to right and return the last after-comma result";


Precedence (Precedence vs Associativity vs Order of evaluation)
---------------------------------------------------------------
-Precedence:
...
-Undefined behavior until C++17 (WHICH is like JS :D):
    ## cout << i << " " << i++;
...
Division Operator and Modulas Operator
--------------------------------------
...
If statement ((The program flow));
----------------------------------
The if else chain
-----------------
-Syntax in C++:
    -- if (condition) to // if to\body is only one statement;
    -- if (condition) { body } // if body is a block of code;
-Syntax in C++17:
    -- if (initialization ; condition) to;
    -- if (initialization ; condition) { body };
    .. But the initializer doesn't seen in the else statement just if and if else ;
    .. "Helps limiting the scope";



=====================
===== CH7: Loop ===== ((The program flow));
=====================
-Loop means circular;
-In our context, it is a way to command the computer to REPEAT the same operations till we ask to STOP;
-With it we can now make any complex logic that requires repetition!;
-Important Loop clauses:
    -- Break clause stops the loop\escapes the other parts and go after that block (loop);
    -- Continue clause tells the compiler escape the other parts and jump to the loop start again and continue from there;
...
-Consists of three important parts:
    01 initialization;
    02 Condition;
    03 Increment\Dec - the step;
    .. Each part wrote on different place not on one place (UNlike the for loop);
    .. Second two parts is for our logic and first for initialization;
    
For Loop
--------
-A way of eligant way to organize a code;
-It is While Loop when we empty both the initialization and the step from it like:
    -- for (; condition ;) { body and the step };
    .. "If we removed the condition then it is the infinite loop - the true for loop :D - stopes it the BREAK clause";


=======================
===== 08 1D Array ===== (());
=======================
-It allows us to define a lot of vars sequently on the memory - on one command - one statement - one var;
-It is a data type that C++ introduces to us - a ds combines a punch of vars;
-Its declaration:
    -- int nums[x];
-Its initialization:
...
-It would give us an RTE - a run time error when - index out of boundry:
    -- When we try to access an array with negative values;
    -- When we try to access an array with an index >= the array size\length;
    .. "UNLESS using the method .at in vector or .. which check before accessing";


=========================
===== 09 Char Arrays=====
=========================
-As we know Every char is evantualy internally is just a number (and the're also consecutive [A-Za and -z]);
    ## cout << (int)'A' // 65;
    ## cout << (int)'a' // 97;
    .. "Char type is internaly a number [-128:127] otherwise => an overflow";

-It is called string - sequence of chars - (char of array);
-It is initialized by several ways:
    01 char name1[7] = { 'L', 'e', 'g', 'e', 'n', 'd'};
    02 char name2[7] = "Legend";
    03 string name3 = "Legend";
    .. "The length we deter should be plus 1 on the size we expect for the terminator nul char (WAY_1_2)";

-The terminator nul char tells the compiler the end of that string so please stop there!;
    -- In syntax 01 02 we just need to plus 1 on the size we expect;
    -- In syntax 00003 we didn't need to plus 1 or even to write it explicitly;
    .. "It is \0";
    .. "The compiler is smart enough to add it whenever the size is added to it by one";

-Reading string with spaces:
    01 getline(cin, name1)    // if we used the data type string name1;
    02 cin.getline(name2, sz) // if we used the data type char name2[50];
    .. "Two ways based on the datatype - first is usually used";

-Array of strings - a 2D - a matrix:
    ## string names[5] {"123", "234", "345"};
    .. "Array of an array of chars";
    .. "Is it end with \0 ?? as last element is about an array auto ends with \0 the ansewr is: (YES) :D";

-Char Arrays vs String Arrays:
    Char Arrays:
    -- It is just an array - a primtive data type from the C-Style; 
    -- We need carfully to handle the nul terminator;
    String Arrays:
    -- It is "not a primitive" !! it is a class;
    -- It is a CPP-Style;
    -- It is a part of the STL library;
    -- It supports a lot of convenient features (e.g. name1 + name2) and comparisons;
    ## string name {"Legend"};

-Some definitions we'll hear in String Arrays:
    -- Prefix: The start of string (index 0 <= length);
    -- Suffix: The end of string (>= 0);
    -- Substring (Subarray): Start wherever and end wherever in consecutive way;
    -- Sub-Sequence: Not consecutive but must be in order (index of next letter is biger than prev index);
...
===== 10 2D Arrays=====
=======================
-It is a matrix (a grid) structure a 2D structure;
-Its Syntax:
    -- int matrix[size_of_rows][size_of_cols];
-Columns row order is slower than Rows col order while prenting:
    -- BEcuase it costs more casch in memory;
-Accessing a val of a row of a specfic column:
    -- matrix[x][y];
    .. X represents the index of row;
    .. Y represents the index of column;
    .. X && Y together represent the val of them (cell);
-It is a flatten array - about a one array - a 1D array on the memory and the formula is:
    -- arr[3][4]:
       1 2 3 4 // 0 ();
       5 6 7 8 // 1 ();
       1 2 3 4 // 2 ();
-The flatten array:
    -- To convert from matrix to 1D:
    .. "Each cell(a, b) =  a*y + b";
    -- To convert from 1D to matrix: a=idx/Y; b=idx%Y;


-Example (Position neighbours):
...
-We can't use a param without a default value after defining one with a default value (CError):
    -- int fun(int a, int b=1, int c) // Wrong;
    -- int fun(int a, int c, int b=1) // Right;
-Paramater casting BEcarful:
    -- When we define a param with int and we pass an argue of double type WILL converted to int;
    .. "TYPE Script ^_^";
-Function overloading:
    -- Difining several funcs with the same name BUT different pars in (Types or count);
    .. "Return isn't considered - and the function type too";
    .. "The compiler finds which one is suitable to use";
-Parameter vs Argument:
    -- Parameter: is the variable which is part of declaration;
    -- Argument: is the default values for those params;
    -- Argument: is the expression used (the values passed) on calling the method;


Pass by reference
-----------------
@REM -- When we define a param with ref and we pass an argue, C WILL use that val and extract its address;
-Passing by variable creating a new memory copy;
-Passing by reference using the same value at the same memory;
-Passing by reference would change the value (as it is not a copy):
    ...
    .. "BUT if the argument is a temporery var (splictly value) WE MUST USE const";
-Arrays always passed by reference:
    -- int ref(arr[]) {};
    .. "DEfault - as the value of arr is its address";
    .. "WE didn't add the & Operator and also we didn't specify the size - auto size";


Builtin functions
-----------------
-Its an already implemented for us and ready to use to save our time (from CTeam or our team);
-#include <cmath>:
    -- abs<int>(-2)         // 2;
    -- fabs<float>(-2.4)    // 2.4;
    -- ceil(2.4)            // 3;
...
The struct methods - Constructor
--------------------------------
-It is a method without a return type + its name is same as class name;
-It is a way to initialize an object from the begin (when we instanciate an obj from it);
-It is called automatically when we define\instanciate a new object;
-It may be defined more than one depends on params (count or type):
    -- full_name();
    -- full_name(string _first, string _last="");
    ## full_name fn_1 = full_name() // Constructor one is the relivent here so will be called;
    ## full_name fn_2 = full_name("Legend", "Ragab") // Constructor two is the relivent here;
    ## full_name fn_3; // No params then constructor one
    .. "Compiler is smart anough to search and fined the relivent one on that case";
    .. "IF we instanciate and object without initialize it .. it will initialized with the empty constructor - Fn3";
    .. "BEcarful: better to use the empty one becuase we may face problems with the params - OOP we'll learn about";


The uniform initialization in C++
---------------------------------
-Did you know that before C++11 we have to use different types of initialization for different cases (no uniform init);
-Did you know that there is more than 19 ways to initialize an int !!;

-It is come to uniform the initialization in C++ after V11;
-It is sometimes more safer like warning us on narrowing conversion;
-It is the only way to solve a problem like in template;

-The copy initialization is when we use an assign:
    ## double copy_init = a_value;
    .. "The value will be created then will be copied to the LHS -- Not eficient";
    ## int narrowing = copy_init;
    .. "The value will be converted from double to int without warning you -- Not secure unlike the uni_init (CW/CE)";
-The dirict initialization is when we don't use an assign (direct pass a value):
...
-Aggregation init in struct (" The Has A relation ");
-At end - simple rules:
    -- Forgot to init then:
        -- Primitive => Garbage
        -- Struct object => Default constructor;
    -- Initialized with something like {} or ():
        -- Primtive => Zero-init;
        -- Struct obj => Default constructor;
    -- Initialized with soemthing like {} or (): type :
        -- using (=) => copy init (ok primtive , slow otherwise(user-defined ));
        -- without (=) => direct init;



========================
===== 16 Templates ===== The function type;
========================
-It is one of the functionalities in C++;
-It has two types the function template and the class template type;
-It is a function that share the same logic once regradless of data type - and called by different types as we need:
    ## One for int, double, float, and so on;
    .. "If we discovered a bug then we'll solve it once";
-It is used by Genreic porgramming;
    -- "Code works in general regradless of the internal details( when we #include) - and the data type too";

-The idea is (&How it works):
    -- Not repeat the same logic by ourself, just to use it with other types;
    .. "BY that we can improve our logic once as the code now is once and in one place";
-Generation:
    .. "INSTEAD of repeating, the compiler will generate (copy\paste) the logic by itself with the called type";
    .. "COMPILER (on compile time) replaces the type words of the template with the corisponding called type";
    .. "Compiler generates ONLY based on the used cases";

-IT is:
    -- Complex and hard to apply;
    -- Give us ugly and hard to understand messages on error messages;
-ITs syntax in making a function template:
    01 template<typename Type1, typename Type2, ..> ;
       function(Type2 x, Type1 y, int z) {};
    02 template<class Type1, class Type2, ..> ;
       function(Type2 x, Type1 y, int z) {};
    .. "WE can use more than type variables";
-It is Called in two ways (Compiler call and the user call :D):
    01 MyTemp(5, 2, 1);
    02 MyTemp<int, int>(5, 2, 1);
    .. "Compiler will guess the type by itself in case we didn't pass different types to the same type var":
    .. "Otherwise => Compiler can't guess - CError";
    .. "In the user-call -- we defining the type explicitly";
...
Static variables
----------------
-One static var for each generated function ( a one for each type generated - regradeless how many used (init once));
    .. "AS the type of var has changed SO new address will created :D";
...
===== 16 3 Variadic ===== (متغير not static);
=========================
